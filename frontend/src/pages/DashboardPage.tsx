...
// --- MOCK DATA ---
const MOCK_HISTORY = [
    { id: '1', title: 'Q3 Financial Audit', date: '10:42 AM', type: 'excel', status: 'active' },
    { id: '2', title: 'User Churn Analysis', date: 'Yesterday', type: 'db', status: 'closed' },
    { id: '3', title: 'Invoice Extraction', date: 'Oct 24', type: 'pdf', status: 'closed' },
    { id: '4', title: 'Server Logs v2.4', date: 'Oct 22', type: 'txt', status: 'closed' },
];
...
export const DashboardPage: React.FC = () => {
    ...
    // --- HANDLERS ---
    const handleNewChat = () => setView('wizard');
    
    const handleLoadChat = (id: string) => {
        setActiveChatId(id);
        setView('chat');
        setMessages([
            { role: 'assistant', content: `**Session Restored.**\n\nAccessing secure context for operation ID: \`${id}\`. Ready for inquiry.` }
        ]);
    };
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!file) return;
        setTimeout(() => {
            setMessages([{ role: 'assistant', content: `**Uplink Established.**\n\nFile: \`${file.name}\`\nSize: \`${(file.size/1024).toFixed(2)} KB\`\n\nI have indexed the schema. You may proceed.` }]);
            setView('chat');
            setActiveChatId('new-session'); 
        }, 800);
    };

    // --- SUB-COMPONENTS ---

    const SidebarItem = ({ item }: { item: typeof MOCK_HISTORY[0] }) => {
        ...
        return (
            ...
            >
                <div className="flex items-center justify-between w-full">
                    ...
                    <span className="text-[9px] text-slate-600 font-mono opacity-60">{item.date}</span>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-0.5">
                        ...
                        {item.status === 'active' && <div className="w-0.5 h-0.5 bg-rose-500 rounded-full animate-pulse"></div>}
                    </div>
                </div>
            </button>
        );
                ...
                <div className="flex-1 overflow-y-auto px-3 py-2 scrollbar-thin space-y-1">
                    {isSidebarOpen && (
                        <div className="flex items-center justify-between px-2 mb-2 mt-2">
                            ...
                            <span className="text-[9px] bg-white/5 text-slate-500 px-1.5 py-0.5 rounded border border-white/5">4</span>
                        </div>
                    )}
                    {MOCK_HISTORY.map(item => (
                         isSidebarOpen ? <SidebarItem key={item.id} item={item} /> : (
                            <div key={item.id} className="w-9 h-9 mx-auto mb-2 rounded-lg bg-white/5 border border-white/5 flex items-center justify-center hover:border-rose-500/50 cursor-pointer text-slate-500 hover:text-rose-500 transition-colors">
